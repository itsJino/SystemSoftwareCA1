...
static ReportFile **previous_files = NULL;
static int previous_file_count = 0;
...
int scan_directory(const char* dir_path, ReportFile** files, int* count) {
    ...
    *files = (ReportFile*)malloc(array_size * sizeof(ReportFile));
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        strncpy((*files)[file_count].filename, entry->d_name, MAX_PATH_LENGTH - 1);
        ...
        /* Get file owner */
        get_file_owner(full_path, (*files)[file_count].department, MAX_USER_LENGTH);
